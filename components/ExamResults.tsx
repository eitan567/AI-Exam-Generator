...
import type { Submission, Exam, Student, StudentAnswer, Question } from '../types';
...
const ExamResults: React.FC<ExamResultsProps> = ({ submission, exam, student, onFinish }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-900 flex flex-col items-center p-4 py-8" dir="rtl">
      <div className="w-full max-w-4xl">
        <header className="text-center bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl p-8 border border-gray-200 dark:border-zinc-800 mb-8">
            <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-2">המבחן הושלם!</h1>
            <p className="text-lg text-gray-600 dark:text-gray-300 mb-6">כל הכבוד, {student.name}! להלן סיכום התוצאות שלך:</p>

            <div className="relative inline-flex items-center justify-center">
                <svg className="w-40 h-40" viewBox="0 0 120 120">
                    <circle className="text-gray-200 dark:text-zinc-700" strokeWidth="10" stroke="currentColor" fill="transparent" r="55" cx="60" cy="60" />
                    <circle
                        className={`${getScoreColor()} transition-all duration-1000 ease-in-out`}
                        strokeWidth="10" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset}
                        strokeLinecap="round" stroke="currentColor" fill="transparent" r="55" cx="60" cy="60"
                        transform="rotate(-90 60 60)"
                    />
                </svg>
                <div className="absolute flex flex-col items-center justify-center">
                    <span className={`text-4xl font-bold ${getScoreColor()}`}>{percentage}%</span>
                    <span className="text-lg text-gray-600 dark:text-gray-300">{score} / {totalPoints} נק'</span>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8 max-w-3xl mx-auto">
                 <div className="bg-gray-100 dark:bg-zinc-800 p-4 rounded-lg text-center">
                    <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 flex items-center justify-center gap-2"><StopwatchIcon className="w-5 h-5"/> זמן ביצוע</h3>
                    <p className="text-2xl font-bold text-gray-800 dark:text-gray-200">{formatDuration(timeTaken)}</p>
                </div>
                <div className="bg-gray-100 dark:bg-zinc-800 p-4 rounded-lg text-center">
                    <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 flex items-center justify-center gap-2"><ClockIcon className="w-5 h-5"/> זמן מוקצב</h3>
                    <p className="text-2xl font-bold text-gray-800 dark:text-gray-200">{exam.duration} דקות</p>
                </div>
                 <div className="bg-gray-100 dark:bg-zinc-800 p-4 rounded-lg text-center">
                    <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400">סטטוס הגשה</h3>
                    <div className={`flex items-center justify-center gap-2 text-2xl font-bold ${status.color}`}>{status.icon} {status.text}</div>
                </div>
            </div>
        </header>
        
        <div className="space-y-6">
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">פירוט תשובות</h2>
                        ...
                        <div className="space-y-3">
                            {q.type === 'open-ended' ? (
                                <div className="space-y-4">
                                    <div>
                                        <h4 className="font-semibold text-gray-600 dark:text-gray-400 mb-2">התשובה שלך:</h4>
                                    ...
                                    return (
                                        <div key={opt.text} className={`flex items-center p-3 rounded-lg border ${colorClass}`}>
                                            <div className="flex-shrink-0 w-8 flex items-center justify-start">
                                                {isStudentChoice && <UserCircleIcon className="w-6 h-6 text-blue-500" title="התשובה שלך"/>}
                                            </div>
                                            {isCorrectOption
        ...
        <div className="mt-8">
            <button
                onClick={onFinish}
                className="btn-cta w-full px-6 py-3 text-black dark:text-white text-lg font-bold rounded-full focus:outline-none focus:ring-4 focus:ring-pink-300"
            >
                סיום וחזרה
            </button>
        </div>
