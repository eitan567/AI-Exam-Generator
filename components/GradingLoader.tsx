...
const gradingMessages = [
    "מנתחים את תשובותיך...",
    "הבינה המלאכותית מעריכה את השאלות הפתוחות...",
    "בודקים מול תשובות המופת...",
    "מחשבים את הציון הסופי...",
    "כמעט סיימנו!"
];
...
const GradingLoader: React.FC = () => {
    ...
    useEffect(() => {
        const intervalId = setInterval(() => {
            setMessageIndex(prevIndex => (prevIndex + 1) % gradingMessages.length);
        }, 2000);
    ...
    return (
        <div className="flex flex-col items-center justify-center h-screen bg-gray-50 dark:bg-zinc-900 text-center p-4">
            ...
            <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-4">בודקים את המבחן שלך...</h2>
            <p className="text-xl text-gray-600 dark:text-gray-300 transition-opacity duration-500 ease-in-out">
                {gradingMessages[messageIndex]}
            </p>
        </div>
    );
};
